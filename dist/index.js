        ...
        switch (selection.actions) {
            ...
            case "Add a department":
                inquirer
                    ...
                    const newDepartment = new Department(departments.length + 1, answer.department);
                    await pool.query("INSERT INTO department (id, name) VALUES ($1, $2)", [newDepartment.id, newDepartment.name]);
                    console.log("Department added!");
                    startApp();
                });
                break;
            case "View all roles":
                pool.query("SELECT employee_role.id, title, department_name AS department, salary FROM employee_role JOIN department ON employee_role.department_id = department.id;", (err, result) => {
                    if (err) {
                        console.error(err);
                        return;
                    }
            ...
            case "Add a role":
                inquirer
                    ...
                    const roles = rows;
                    const newRole = new Role(roles.length + 1, response.newRole, response.salary, departmentId());
